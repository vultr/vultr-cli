...
// BareMetalIPv4Printer ...
type BareMetalIPv4Printer struct {
	IPv4 []govultr.IPv4 `json:"ipv4s"`
	Meta *govultr.Meta  `json:"meta"`
}

// JSON ...
func (b *BareMetalIPv4Printer) JSON() []byte {
	return printer.MarshalObject(b, "json")
}

// YAML ...
func (b *BareMetalIPv4Printer) YAML() []byte {
	return printer.MarshalObject(b, "yaml")
}

// Columns ...
func (b *BareMetalIPv4Printer) Columns() [][]string {
	return [][]string{0: {
		"IP",
		"NETMASK",
		"GATEWAY",
...
// Data ...
func (b *BareMetalIPv4Printer) Data() [][]string {
	var data [][]string
	for i := range b.IPv4 {
		data = append(data, []string{
			b.IPv4[i].IP,
...
// Paging ...
func (b *BareMetalIPv4Printer) Paging() [][]string {
	return printer.NewPaging(b.Meta.Total, &b.Meta.Links.Next, &b.Meta.Links.Prev).Compose()
}

// ======================================

// BareMetalIPv6Printer ...
type BareMetalIPv6Printer struct {
	IPv6 []govultr.IPv6 `json:"ipv6s"`
	Meta *govultr.Meta  `json:"meta"`
}

// JSON ...
func (b *BareMetalIPv6Printer) JSON() []byte {
	return printer.MarshalObject(b, "json")
}

// YAML ...
func (b *BareMetalIPv6Printer) YAML() []byte {
	return printer.MarshalObject(b, "yaml")
}

// Columns ...
func (b *BareMetalIPv6Printer) Columns() [][]string {
	return [][]string{0: {
		"IP",
		"NETWORK",
		"NETWORK SIZE",
...
// Data ...
func (b *BareMetalIPv6Printer) Data() [][]string {
	var data [][]string
	for i := range b.IPv6 {
		data = append(data, []string{
			b.IPv6[i].IP,
			b.IPv6[i].Network,
			strconv.Itoa(b.IPv6[i].NetworkSize),
			b.IPv6[i].Type,
		})
	}
...
// Paging ...
func (b *BareMetalIPv6Printer) Paging() [][]string {
	return printer.NewPaging(b.Meta.Total, &b.Meta.Links.Next, &b.Meta.Links.Prev).Compose()
}
